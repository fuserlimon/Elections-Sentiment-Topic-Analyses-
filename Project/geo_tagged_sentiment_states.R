...
load("tweets_all_sent_mapped_state.Rdata")
...
sentiment_state <- function(filename, name_csv){
  ...
  for(j in 1:length(States)){
    ...
    sentiment_scores[j, "Positives"] <- pos_count
    sentiment_scores[j, "Negatives"] <- neg_count
...
BS_states_sentiment <- sentiment_state(BS, "polarity_state_BS.csv")
