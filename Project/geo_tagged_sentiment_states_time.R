...
load("tweets_all_sent_mapped_state.Rda")
...
sentiment_state <- function(filename, name_csv){
  ...
  States <- unique(filename$state)
  
  sentiment_scores_state <- data.frame()
  for(j in 6:7){
    subset <- filename[filename$state == States[j], ]
    for(i in 1:length(Dates)){
      tc <- tc(subset[subset$Date == Dates[i] , ])
      pos_count <- sum(str_count(tc, positive.words), na.rm = T)
      neg_count <- sum(str_count(tc, negative.words), na.rm = T)
      sentiment_scores[i, "Date"] <- as.character(Dates[i])
      sentiment_scores[i, "Positives"] <- pos_count
      sentiment_scores[i, "Negatives"] <- neg_count
      print(i)
    }
    sentiment_scores$state <- States[j]
    sentiment_scores_state <- rbind(sentiment_scores_state, sentiment_scores)
    print(paste("State no ", j))
    print(States[j])
  }
  
  write.csv(sentiment_scores_state, file = name_csv)
  return(sentiment_scores_state)
}

load("data/BS.Rdata")

BS_6_10 <- sentiment_state(BS, "polarity_state_BS.csv")
